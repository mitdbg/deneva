...
{
  bool success = false;
  // TODO may use mm_malloc to avoid false sharing
  QueueEntry * new_head = (QueueEntry *) malloc(sizeof(QueueEntry));
  //QueueEntry * new_head = (QueueEntry *) _mm_malloc(sizeof(QueueEntry));
  ...
  if (old_head == NULL)
    ...
    //assert(ATOM_CAS(old_head->next, NULL, new_head));
...
LockfreeQueue::dequeue(uintptr_t & value)
{
  ...
  while (!success) {
    ...
    if (old_tail != old_head && old_tail->next != NULL)
      success = ATOM_CAS(_tail, old_tail, old_tail->next);
    else if (old_tail == old_head) {
      success = ATOM_CAS(_head, old_head, NULL);
      if (success)
        success = ATOM_CAS(_tail, old_tail, NULL);
    }
    if (!success)
      PAUSE
  }
