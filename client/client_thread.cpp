...
//#if !TPORT_TYPE_IPC
    ...
    printf("responses to wait for: %d\n",rsp_cnt);
    fflush(stdout);
    while(rsp_cnt > 0) {
		m_query = tport_man.recv_msg();
        if(m_query != NULL && m_query->rtype == INIT_DONE) {
            rsp_cnt --;
            printf("received response. responses left: %d\n",rsp_cnt);
            fflush(stdout);
        } else if(m_query != NULL) {
            printf("Received a new query (not INIT_DONE)\n");
            fflush(stdout);
...
RC Client_thread_t::run() {
	...
	pthread_barrier_wait( &warmup_bar );
	stats.init(get_thd_id());
    printf("after barrier, thd_id=%lu\n", _thd_id);
    fflush(stdout);
...
//#if !TPORT_TYPE_IPC
    if( _thd_id == 0) {
        for(uint64_t i = 0; i < g_node_cnt+g_client_node_cnt; i++) {
            if(i != g_node_id) {
                //rem_qry_man_cl.send_init_done(i);
                rem_qry_man.send_init_done(i);
                printf("Sent INIT_DONE to node: %lu\n", i);
                fflush(stdout);
            }
        }
    }
...
//            default:
//                printf("received this type of query: %d\n",m_query->rtype);
//                break;
//        case RPASS:
