...
void RemoteQueryClient::init(uint64_t node_id, workload * wl) {
	q_idx = 0;
	_node_id = node_id;
	_wl = wl;
...
#endif
void RemoteQueryClient::send_init_done(uint64_t dest_id) {
  uint64_t total = 2;
	void ** data = new void *[total];
	int * sizes = new int [total];
  int num = 0;
...
void RemoteQueryClient::send_init(base_query * query,uint64_t dest_part_id) {
...
void RemoteQueryClient::send_remote_query(uint64_t dest_id, void ** data, int * sizes, int num) {
