...
BaseQuery * YCSBQueryGenerator::create_query(Workload * h_wl, uint64_t home_partition_id) {
  ...
  BaseQuery * query;
  if (SKEW_METHOD == HOT) {
    query = gen_requests(home_partition_id, h_wl);
  } else
    assert(false);
...
BaseQuery * YCSBQueryGenerator::gen_requests(uint64_t home_partition_id, Workload * h_wl) {
  YCSBQuery * query = new YCSBQuery;
